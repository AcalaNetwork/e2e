...
export const startAcalaNode = async (ws_port = 19933): Promise<ChildProcess> => {
	...
	await new Promise<void>((resolve, reject) => {
		const timer = setTimeout(() => {
            let error_msg = `Failed to start Acala Node.\n`;
            error_msg += `Command: ${cmd} ${args.join(" ")}\n`;
            error_msg += `Logs:\n`
            error_msg += binaryLogs.map((chunk) => chunk.toString()).join("\n");
            binary.kill();
            reject(error_msg);
		}, SPAWNING_TIME - 2000);
